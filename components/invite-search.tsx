...
import {CheckCircle, Mail, Search, XCircle} from "lucide-react"
...
export function InviteSearch({eventId}: InviteSearchProps) {
    ...
    const handleSearch = async (e: React.FormEvent) => {
        ...
        try {
            const response = await getInviteeByToken(name)
            const rsvp = await getRsvpByInviteeId(response.id)
            ...
            setResult({
                ...
                invitee: {
                    ...
                    token: response.uniqueToken,
                    hasRsvped: !!rsvp,
                    rsvpStatus: rsvp ? rsvp.status : undefined,
                                    ...
                                    {result.invitee.hasRsvped ? (
                                        <div className="pt-2">
                                            <p className="text-sm text-muted-foreground">
                                                ...
                                                <span
                                                    className="font-semibold text-foreground capitalize">{parseRsvpStatus(result.invitee.rsvpStatus)}</span>
                                            </p>
                                        </div>
                                    ) : (
                                        <div className="pt-2">
                                            <p className="text-sm text-muted-foreground">
                                                You haven't responded yet. Please RSVP using your personalized
                                                invitation.
                                            </p>
                                        </div>
                                    )}
                        ...
                        <div className="flex items-start gap-3">
                            ...
                            <div className="flex-1">
                                ...
                                <p className="text-muted-foreground mt-1">
                                    We couldn't find an invitation for "{name}". Please check the spelling or contact
                                    the event organizer.
                                </p>
                            </div>
                        </div>
