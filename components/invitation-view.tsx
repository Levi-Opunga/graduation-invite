...
export function InvitationView({invitee, event, rsvp}: InvitationViewProps) {
    ...
    if (isSubmitted) {
        return (
            <main
                className="min-h-screen flex items-center justify-center px-4 py-16 bg-gradient-to-br from-slate-50 via-white to-slate-50">
                ...
                <Card
                    className="max-w-2xl w-full p-12 text-center space-y-3 rounded-3xl shadow-2xl border-0 relative overflow-hidden bg-white/80 backdrop-blur-sm">
                    <div className="absolute top-0 left-0 right-0 h-2 bg-gradient-to-r"
                         style={{backgroundImage: `linear-gradient(90deg, ${primaryColor}, ${accentColor})`}}/>

                    <div className="flex justify-center">
                        <div
                            className="w-20 h-20 rounded-full flex items-center justify-center relative animate-puls"
                            style={{backgroundColor: `${accentColor}15`}}
                        >
                            {/*<div className="absolute inset-0 rounded-full animate-ping opacity-20"*/}
                            {/*     style={{backgroundColor: accentColor}}/>*/}
                            <CheckCircle2 className="w-12 h-12 relative z-10" style={{color: accentColor}}/>
                        </div>
                    </div>
    ...
    return (
        <main className="min-h-screen px-4 py-16 bg-gradient-to-br from-slate-50 via-white to-slate-50 relative">
            <div className="absolute inset-0 overflow-hidden pointer-events-none">
                <div className="absolute top-32 -left-20 w-96 h-96 rounded-full opacity-5 blur-3xl"
                     style={{background: primaryColor}}/>
                <div className="absolute bottom-32 -right-20 w-96 h-96 rounded-full opacity-5 blur-3xl"
                     style={{background: accentColor}}/>
            </div>
            ...
            <div className="max-w-4xl mx-auto space-y-12 relative z-10">
                ...
                <div className="text-center space-y-8">
                  ...
                  {invitee.name}
                                    <span className="absolute -bottom-1 left-0 right-0 h-0.5 opacity-30"
                                          style={{background: accentColor}}/>
                </span>
                ...
                {/* Event Details */}
                <Card
                    className="relative overflow-hidden p-10 space-y-2 rounded-3xl border border-gray-400/20 shadow-xl bg-white/80 backdrop-blur-sm"
                >
                    {/* Subtle Accent Glow */}
                    <div
                ...
                {/* RSVP Form */}
                <Card
                    className="p-10 border rounded-3xl shadow-xl bg-white/80 backdrop-blur-sm overflow-hidden relative border-gray-400/20">
                    <div className="absolute bottom-0 left-0 w-64 h-64 rounded-full opacity-5 blur-3xl -ml-32 -mb-32"
                         style={{background: primaryColor}}/>
                                    ...
                                    {[
                                        ...
                                        <Label
                                            ...
                                            className={`flex items-center space-x-3 p-4 rounded-2xl border-2 transition-colors cursor-pointer ${
                                                formData.status === value ? "border-current" : "border-slate-200"
                                            }`}
                                            style={{
                                                borderColor: formData.status === value ? accentColor : undefined,
                                            }}
                                            htmlFor={value}
                                        >
                                ...
                                <>
                                    ...
                                    <div className="space-y-3">
                                        <Label htmlFor="dietary" className="text-base font-semibold"
                                            ...
                                            className="text-slate-400 font-normal">(Optional)</span>
                                        </Label>
                                        <Input
                                            id="dietary"
